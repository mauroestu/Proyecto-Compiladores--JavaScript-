document.write('<script src="Debugg/objetos3d.js" type="text/javascript"></script>');
    ...
    try
    {
        ...
        else
        {
            ...
            if(recoleccion){
                ...
                NumeroIngresado = prompt(strMensaje);
            }
...
function instr3d(raiz){
    try
    {
        ...
        else
        {
            ...
            if(recoleccion){
                CadenaIngresada = 'probando';
            }
            else{
                CadenaIngresada = prompt(strMensaje);
            }
...
function etiqueta3d(raiz){
    try
    {
        if(recoleccion)
        {
            ...
            ReturnEtiqueta = strNombreEtiqueta;
            if(numEtiqueta == UltEtiqueta){
                EjecutandoSalto = false; 
                UltEtiqueta = -1;
                
            }
            AgregaEtiqueta(strNombreEtiqueta,raiz);
        }
        else
        {
            Fila = raiz.Fila + 1;
            ...
            if(Sym3d[Sym3d.length - 1].EtqRetorno == strNombreEtiqueta){
                Sym3d.pop(); 
            }
        }
    ...
    try
    {
        if(recoleccion)
        {
            ...
            if(ptnCuerpoMetodo != null)
            {
                ...
                Sym3d.pop();
                AgregaEtiquetaRetorno(strNombreMetodo,ReturnEtiqueta);
                ReturnEtiqueta = '';
            }
        }
        else
        {
            Fila = raiz.Fila + 1;
            SeleccionarFila(Fila);
            var ptnCuerpoMetodo = null, strNombreMetodo = raiz.Nodes[0].FindTokenAndGetText(), EtqRetorno = '';
            for(var i = 0; i < Sym3d[0].listContenido.length; i++)
            {
                if(strNombreMetodo == Sym3d[0].listContenido[i].strNombre && Sym3d[0].listContenido[i].IsMetodo)
                {
                    ptnCuerpoMetodo = Sym3d[0].listContenido[i].CuerpoMetodo; EtqRetorno = ptnCuerpoMetodo = Sym3d[0].listContenido[i].Valor;
                    break;
                }
            }
            
            if(ptnCuerpoMetodo != null) Sym3d.push(new ObjSym3d(con_metodo,[],EtqRetorno)); 
        }
    }
    catch(err)
    {
...
//---- Métodos auxiliares, no necesitan recolección.
